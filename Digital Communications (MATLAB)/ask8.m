...
sig_demod = sig_dsb .* loc_osc;

% Χαμηλοπερατό φίλτρο Butterworth
order = 5;
fcut = 4000;
fcenter = 0;
[sig_butterworth, ~] = butterworth_filter(sig_demod, 1/Fs, order, fcut, fcenter);

% Γραφική παράσταση του αποδιαμορφωμένου σήματος
subplot(2,1,2);
plot(t2, sig_butterworth);
xlabel('Χρόνος (s)');
ylabel('Πλάτος');
title('Αποδιαμορφωμένο σήμα (butterworth filter)');

% Ακούστε το αποδιαμορφωμένο σήμα
% sound(real(double(sig_butterworth)), Fs);
audiowrite('sound3.wav',real(double(sig_butterworth)),Fs);
